# ADR 0008 — Provider-Neutral Explainable Recommendations
...
Implement recommendations behind a provider-neutral gateway. The application layer supplies a bounded recommendation context derived from the authenticated owner's wardrobe, profile, outfits, wear aggregates, and explicit request. Provider output must be parsed into a strict versioned schema before it reaches the domain or UI.

Every accepted recommendation includes:

- a generated recommendation identifier;
- the authenticated owner identifier, retained internally and excluded from provider context;
- the explicit user request or occasion;
- referenced wardrobe item identifiers;
- concise reasoning tied to those items and the user's controls;
...
When no provider is configured, a provider fails, output is invalid, confidence is too low, or ownership validation fails, Sartoria returns a deterministic wardrobe-first fallback. The fallback reuses a suitable saved outfit when possible or composes a restrained suggestion from owned, available items using explicit rules. It clearly identifies fallback provenance and never fabricates wardrobe facts.

## Data minimisation

The gateway receives only the minimum context required for the request. Private notes, wear-event notes, media object keys, exact account identifiers, and unrelated history are excluded. Measurements are included only when recommendation consent is enabled in the private style profile.

## Security and privacy controls

- resolve owner identity only on the server;
- never send database credentials, storage keys, session tokens, or internal object keys to a provider;
- validate all model output against a strict schema;
- verify all referenced item identifiers against the owner-scoped context;
- cap provider request and response bodies at 64 KiB;
- enforce HTTPS and bounded timeouts in production provider mode;
- store provider-neutral metadata rather than raw hidden reasoning;
- do not persist chain-of-thought;
- keep request text and profile content out of logs and diagnostics;
- provide correction, rejection, and deletion controls;
...
- user corrections become durable private product signals;
...
- provider evaluation and operations require additional release controls.
...
Set `SARTORIA_RECOMMENDATION_MODE=fallback` or disable the recommendation navigation and route surfaces. Preserve stored recommendation and correction records unless an approved deletion or forward migration is executed.
